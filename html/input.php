...
<head>
  ...
  <meta name="viewport" content="width=device-width, initial-scale=0.8,
		minimum-scale=0.8, maximum-scale=0.8, user-scalable=yes" />
...
if (!isset($_POST['sent'])) {
    ?>
        <form method="post" action="">
          ...
          <div class="heading">
            ...
            <center>
              <li id="repass"><a href="dlq.zip" style="color:#222222">登陆器下载</a>
    ...
    if (mysqli_connect_errno()) {
        echo "<div class=\"error\">不能连接数据库</div>";
        echo "<div class=\"failed\"><b>注册失败.</b></div>
	      <center>
        ...
        if (empty($_POST['login']) || empty($_POST['pass']) || empty($_POST['pass2']) || empty($_POST['mail'])) {
            ...
            $sql_acc = "SELECT username FROM account WHERE username = UPPER('$_POST[login]')";
            $getacc = mysqli_query($db, $sql_acc);
            $account = mysqli_fetch_array($getacc);
            if (strcasecmp($_POST['login'], $account['username']) == 0) {
            ...
            if (!IsValidEmail($_REQUEST['mail'])) {
                echo "<div class=\"error\">你输入的电子邮件格式不对，请提供一个有效的电子邮件！</div>";
                $passed = false;
            } else {
                if ($oneaccpermail == true) {
                    $sql_mail = "SELECT Count(1) FROM account WHERE email = '$_POST[mail]'";
                    $getmail = mysqli_query($db, $sql_mail);
                    $mail = mysqli_fetch_array($getMail);
                    if ($mail !== 0) {
            ...
            if ($passed == true) {
                $sql_insert = "INSERT INTO account (`username`,`sha_pass_hash`, `email`, `expansion`) VALUES (UPPER('$_POST[login]'), SHA1(CONCAT(UPPER('$_POST[login]'),':',UPPER('$_POST[pass]'))), '$_POST[mail]', '$_POST[expansion]')";
                mysqli_query($db, $sql_insert);
                mysqli_close($db);
                echo "<div class=\"done\">你已在" . $servername . "注册成功。<br><br><br><br></div>";
                echo "<div class=\"finished\"><b>注册成功.</b></div>
                <center>
               <button class=\"homepage\" onclick=\"window.location.href='$homepage';\">返回主页</button>
                </center> ";
            } else {
                echo " <div class=\"failed\"><b>注册失败.</b></div>  <center>                                                                  <button class=\"homepage\" onclick=\"window.location.href='$page';\">重新注册</button></center>  ";}
        }
    }
}
?>
