...
if (!isset($_POST['sent'])) {
    ?>
        <form method="post" action="">
          ...
          <div class="inputwrap">
            <input type="mail" name="mail" size="39">
          </div>
    ...
    if (mysqli_connect_errno()) {
        ...
        if (empty($_POST['login']) || empty($_POST['pass1']) || empty($_POST['pass2']) || empty($_POST['mail'])) {
            ...
            $sql = "select * from account where username = '$username' and email='" . $mail . "'";
            $getomail = mysqli_query($db, $sql);
            $omail = mysqli_fetch_assoc($getomail);
            if (!$omail) {
            ...
            if ($passed) {
                $sql_fipass = "UPDATE account SET sessionkey=0 , v=0 , s=0 , sha_pass_hash = '" . $sha1newpass . "' WHERE username = '$username'";
                mysqli_query($db, $sql_fipass);
